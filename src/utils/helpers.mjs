...
export const maskEmail = (email) => {
  ...
  const maskedLocal =
    localPart.length <= 4
      ? `${firstChar}***`
      : `${firstChar}${'*'.repeat(localPart.length - 2)}${lastChar}`;
...
export const generateRandomCode = () => {
  ...
  const code = crypto.randomInt(0, 1_000_000).toString().padStart(6, '0'); // ensures exactly 6 digits
...
export const hasRole = (user, roles = []) => {
  if ((!user) instanceof User || !user.roles || !Array.isArray(roles))
    return false;
...
export const pcFetch = asyncHandler(async (path, init = {}) => {
  ...
  try {
    ...
    throw new Error(
      `Invalid response from PayChangu: ${text.substring(0, 100)}`,
    );
  }
